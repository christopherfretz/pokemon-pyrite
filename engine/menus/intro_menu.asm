...
.Check2Pass:
	...
	; M13-Q5: Continue_MobileAdapterMenu dropped (a no-op: CheckMobileAdapterStatus always returns nc)
	call CloseWindow
	call ClearTilemap
	ld c, 20
	call DelayFrames
...
StartTitleScreen:
; Kanto hack (M12c): Yellow's title screen (engine/movie/title.asm) runs as one
; blocking call and returns the player's choice in wTitleScreenSelectedOption.
	ldh a, [rWBK]
	push af
	ld a, BANK(wTitleScreenBlinkTimer)
	ldh [rWBK], a
