...
; Kanto hack (M6 9aa): Route18_Blocks moved to "Map Blocks 3" -- the re-cut
; ROUTE 18 is 25x9 = 225 bytes, and "Map Blocks 2" has nothing like that free.
...
; Kanto hack (M7 10a): Yellow's Route15Gate2F.blk is byte-identical to its
; Route11Gate2F.blk as well, so it shares the file (the Route12/16/18 trick).
...
; Kanto hack (M6 9aa): Yellow's Route18Gate2F.blk is byte-identical to its
; Route11Gate2F.blk too, so the 9o placeholder maps/Route18Gate2F.blk is gone
; and this INCBINs the shared file (the Route12Gate2F / Route16Gate2F trick).
	INCBIN "maps/Route11Gate2F.blk"

; Kanto hack (M6 9aa): ROUTE 18 re-cut to Yellow's 25x9 (was Crystal's 10x9
; stub in "Map Blocks 2"); generated by scripts/celadon_blk.py.
Route18_Blocks:
...
; Kanto hack (M7 10a): FUCHSIA MART is 4x4 in Yellow, not the 6x4 of Crystal's
; shared Mart.blk, and FUCHSIA's POKeCENTER 1F is 7x4, not the shared 5x4, so
...
; dimensions -- the four areas are all-FLOOR TILESET_KANTO ($0a), the gate and
; the rest houses all-FLOOR TILESET_KANTO_GATE ($7f, the usable twin of the gate
; tileset's unusable block $00, M6 9o), the secret house all-FLOOR TILESET_LAB
