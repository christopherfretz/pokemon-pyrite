; Yellow's pikapic object framesets, transcribed verbatim from
; vendor/pokeyellow/data/pikachu/pikachu_pic_objects.asm
; (docs/PIKACHU-EMOTIONS.md A5 step E2).  The pikaanim_def / pikaframe /
; pikaframedelay / pikaframeend macros moved to macros/scripts/pikapic.asm.
;
; A frameset is a list of 2-byte frames: (tilemap index, duration in frames).
; `pikaframedelay N` is `db 0, N` -- tilemap 0 is PikaAnimTilemap_0, which
; draws nothing, so it is a blank frame.  `pikaframeend` is `db $e0`.
